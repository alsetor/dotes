...
export class DotesService {
  ...
  getTagsByTemplateUid(uid: number): Observable<Tag[]> {
    const endpoint = `${url}/GetTagsByTemplateUid/${uid}`;

    return this.http.get<any>(endpoint, { headers: this.headers })
      .pipe(catchError(this.handleError('GetTagsByTemplateUid', null)));
  }

  generateDocument(uid: string, tags: TagValue[], name: string, fileName: string) {
    const endpoint = `${url}/GenerateDocument`;

    return this.http
      .post<any>(endpoint, { templateUid: uid, tags: tags }, { headers: this.headers })
      .subscribe(
        (result) => {
          if (result) {
