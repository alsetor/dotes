...
export class CreateDocumentByTemplateComponent implements OnInit, OnDestroy {
  ...
  getTemplate(): void {
    this.templateService.getTemplate(this.id).pipe(takeUntil(this.ngUnsubscribe))
      .subscribe((template) => {
        this.dotesService.getTagsByTemplateUid(template.uid).subscribe(tags => {
          template.tags = tags;
  ...
  generateDocument() {
    ...
    this.dotesService.generateDocument(this.template.uid, tags, this.template.name, this.template.fileName);
  }
}
